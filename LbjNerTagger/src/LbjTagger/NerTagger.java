...
public class NerTagger {
	public static void main(String[] args){
		try{
			...
			// added by Tathagata to make tagging work for all files in a directory
			// originial line 	NETagPlain.tagFile(args[1], args[2],false);
			// Expects the first parameter to be the input directory (to be tagged)
			// and second parameter to be the path to the output directory (to place
			// the tagged file)

				java.io.File dir = new java.io.File(args[1]);
				if(dir.isDirectory()){
					for (java.io.File f : dir.listFiles()){
						int son = f.toString().lastIndexOf(java.io.File.separator)+1;
						NETagPlain.tagFile(f.toString(), args[2] + f.toString().substring(son,f.toString().length()),false);
					}
				}
